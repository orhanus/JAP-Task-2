    ...
    public partial class AddProcedures : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            string GetTopMostRatings = @"CREATE OR ALTER PROCEDURE spGetTopMostRatings
                                            AS
                                            BEGIN
                                                SELECT TOP 10 dbo.Shows.Id as MovieId, Title, Count(Score) as NumberOfRatings, AVG(CAST(Score as FLOAT)) as AverageRating
                                                FROM dbo.Shows
                                                INNER JOIN dbo.Ratings
                                                ON dbo.Shows.Id = dbo.Ratings.ShowId
                                                GROUP BY dbo.Shows.Id, Title
                                                HAVING Count(Score) >= (SELECT COUNT(Score)
                                                    FROM dbo.Ratings
                                                    INNER JOIN dbo.Shows
                                                    ON dbo.Shows.Id = dbo.Ratings.ShowId
                                                    GROUP BY dbo.Shows.Id
            ...
            string GetTopWithMostScreenings = @"CREATE OR ALTER PROCEDURE spGetTopWithMostScreenings @StartDate DATETIME, @EndDate DATETIME
                                                AS
                                                BEGIN
                                                    SELECT TOP 10 dbo.Shows.Id AS MovieId, dbo.Shows.Title, COUNT(dbo.Screenings.Id) AS ScreeningCount
                                                    FROM dbo.Shows
                                                    INNER JOIN dbo.Screenings
                                                    ON dbo.Shows.Id = dbo.Screenings.MovieId
                                                    WHERE dbo.Screenings.ScreeningTime BETWEEN @StartDate and @EndDate
                                                    GROUP BY dbo.Shows.Id, dbo.Shows.Title
            ...
            string GetTopWithMostSoldTicketsWithoutRating = @"CREATE OR ALTER PROCEDURE spGetTopWithMostSoldTicketsWithoutRating
                                                                AS
                                                                BEGIN
                                                                    SELECT dbo.Shows.Id AS MovieId, dbo.Shows.Title, dbo.Screenings.Id AS ScreeningId, COUNT(dbo.ScreeningUser.ScreeningsId) AS TicketsSold
                                                                    FROM dbo.Shows
                                                                    ...
                                                                    GROUP BY dbo.Shows.Id, dbo.Shows.Title, dbo.Screenings.Id
                                                                END";
